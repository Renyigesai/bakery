    ...
    public static final RegistryObject<CreativeModeTab> BAKERY_TAB = REGISTER.register("bakeries_tab", () ->
            CreativeModeTab.builder().icon(() -> new ItemStack(BakeriesBlocks.OVEN.get()))
                    ...
                    .displayItems((itemDisplayParameters, output) -> {
                        ...
                        output.accept(BakeriesItems.MOULD_TWO.get());                // 模具二号
                        output.accept(BakeriesItems.ROUND_MOULD.get());              // 月饼模具
                        ...
                        output.accept(BakeriesItems.MOONCAKE.get());                 // 月饼
                        ...
                        output.accept(BakeriesItems.CHEESE_CREAM_BREAD.get());       // 奶酪奶油面包
                        ...
                        output.accept(BakeriesItems.HONEY_BUTTER_SPREAD_COUNTRY_BREAD.get()); // 蜂蜜黄油乡村面包
                        ...
                        output.accept(BakeriesItems.BROWN_SUGAR_LATTE.get());        // 黑糖拿铁
    ...
    public static final RegistryObject<CreativeModeTab> BAKERY_SEMI_MANUFACTURED_PRODUCT_TAB = REGISTER.register("bakery_semi_manufactured_product_tab", () ->
            CreativeModeTab.builder().icon(() -> new ItemStack(BakeriesItems.BAGEL_DOUGH.get()))
                    ...
                    .displayItems((itemDisplayParameters, output) -> {
                        ...
                        // ========== 1. 桶装原料 ==========
                        output.accept(BakeriesItems.CAKE_PASTE_BUCKET.get());        // 蛋糕糊桶
                        output.accept(BakeriesItems.EGG_YOLK_PASTE_BUCKET.get());    // 蛋黄糊桶
                        output.accept(BakeriesItems.FOAMED_PROTEIN_BUCKET.get());    // 发泡蛋白桶

                        // ========== 2. 基础面团（及其发酵版本） ==========
                        output.accept(BakeriesItems.SWEET_DOUGH.get());              // 甜面团
                        conditionOutput(output, BakeriesItems.SWEET_DOUGH_FERMENTATION.get(), BakeriesConfig.fermentationGameplay);

                        output.accept(BakeriesItems.COCOA_DOUGH.get());              // 可可面团
                        conditionOutput(output, BakeriesItems.COCOA_DOUGH_FERMENTATION.get(), BakeriesConfig.fermentationGameplay);

                        output.accept(BakeriesItems.SALTED_DOUGH.get());             // 盐面团
                        conditionOutput(output, BakeriesItems.SALTED_DOUGH_FERMENTATION.get(), BakeriesConfig.fermentationGameplay);

                        output.accept(BakeriesItems.WHOLE_WHEAT_DOUGH.get());        // 全麦面团
                        conditionOutput(output, BakeriesItems.WHOLE_WHEAT_DOUGH_FERMENTATION.get(), BakeriesConfig.fermentationGameplay);

                        output.accept(BakeriesItems.CRISPY_DOUGH.get());             // 酥皮面团
                        output.accept(BakeriesItems.PASTRY.get());                   // 油酥

                        // ========== 3. 蛋挞 ==========
                        output.accept(BakeriesItems.EGG_TART_SHELL.get());           // 蛋挞壳
                        output.accept(BakeriesItems.RAW_EGG_TART.get());             // 生蛋挞

                        // ========== 4. 司康 / 月饼 ==========
                        output.accept(BakeriesItems.SCONE_DOUGH.get());              // 司康面团
                        output.accept(BakeriesItems.RAW_MOONCAKE.get());             // 生月饼
                        output.accept(BakeriesItems.RAW_PORK_MOONCAKE.get());        // 生鲜肉月饼

                        // ========== 5.（模具类） ==========
                        output.accept(BakeriesItems.MOULD_TOAST_DOUGH.get());        // 模具吐司面团
                        output.accept(BakeriesItems.MOULD_CHEESE_COCOA_TOAST_DOUGH.get()); // 模具奶酪可可吐司面团
                        output.accept(BakeriesItems.MOULD_CAKE_PASTE.get());         // 模具蛋糕糊
                        output.accept(BakeriesItems.MOULD_CARROT_CAKE_PASTE.get());  // 模具胡萝卜蛋糕糊
                        output.accept(BakeriesItems.MOULD_BASQUE_CAKE_PASTE.get());  // 模具巴斯克蛋糕糊
                        output.accept(BakeriesItems.MOULD_RED_VELVET_CAKE_PASTE.get());// 模具红丝绒蛋糕糊
                        output.accept(BakeriesItems.MOULD_MATCHA_CAKE_PASTE.get());  // 模具抹茶蛋糕糊
                        output.accept(BakeriesItems.MOULD_POUND_CAKE_PASTE.get());   // 模具磅蛋糕糊
                        output.accept(BakeriesItems.PAPER_CUP_CAKE_PASTE.get());     // 纸杯蛋糕糊

                        // ========== 6. 面包面团 ==========
                        output.accept(BakeriesItems.BAGEL_DOUGH.get());              // 贝果面团
                        output.accept(BakeriesItems.WHOLE_WHEAT_BAGEL_DOUGH.get());  // 全麦贝果面团
                        output.accept(BakeriesItems.ROUND_BREAD_DOUGH.get());        // 圆面包面团
                        output.accept(BakeriesItems.BERRY_BREAD_DOUGH.get());        // 莓果面包面团
                        output.accept(BakeriesItems.BROWN_SUGAR_ROLL_DOUGH.get());   // 红糖卷面团
                        output.accept(BakeriesItems.PINEAPPLE_BUN_DOUGH.get());      // 菠萝包面团
                        output.accept(BakeriesItems.CROISSANT_DOUGH.get());          // 可颂面团
                        output.accept(BakeriesItems.SALT_CROISSANT_DOUGH.get());     // 盐可颂面团
                        output.accept(BakeriesItems.BAGUETTE_DOUGH.get());           // 法棍面团
                        output.accept(BakeriesItems.CIABATTA_DOUGH.get());           // 恰巴塔面团
                        output.accept(BakeriesItems.FOCACCIA_DOUGH.get());           // 佛卡夏面团
                        output.accept(BakeriesItems.COUNTRY_BREAD_DOUGH.get());      // 乡村面包面团

                        // ========== 7. 派 / 披萨 ==========
                        output.accept(BakeriesItems.CREAM_PUMPKIN_PIE_DOUGH.get());  // 奶油南瓜派面团
                        output.accept(BakeriesItems.PIZZA_FLATBREAD.get());          // 披萨饼底
                        output.accept(BakeriesItems.RAW_PIZZA.get());                // 生披萨
                        output.accept(BakeriesItems.RAW_CUSTOM_PIZZA.get());         // 生自定义披萨
                    })
                    .build());
