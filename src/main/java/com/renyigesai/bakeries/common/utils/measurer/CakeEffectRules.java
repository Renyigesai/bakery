...
public class CakeEffectRules {
    ...
    public static List<MobEffectInstance> tooDisgusting(List<MobEffectInstance> rule){
        ...
        if (effects.contains(BakeriesMobEffects.BERRY_SOUR) && effects.contains(BakeriesMobEffects.TEA_ASTRINGENT) && effects.contains(BakeriesMobEffects.COCOA_MANIA)){
            int end = rule.size() - 1;

            Holder<MobEffect> effect = rule.get(end).getEffect();
