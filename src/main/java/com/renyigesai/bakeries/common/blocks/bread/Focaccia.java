...
public class Focaccia extends BreadBlock {
    @Override
    public List<VoxelShape> createShapes(@NotNull BlockState state, @NotNull BlockGetter level, @NotNull BlockPos pos, @NotNull CollisionContext context) {
        ...
        if (value == 1) {
            shapes.add(rotateBox(5.5d,0,5.5d,5,2.8d,5, facing));
        } else if (value == 2) {
            shapes.add(rotateBox(2.5d,0,4.5d,11,2.8d,6, facing));
        } else if (value == 3) {
            shapes.add(rotateBox(2.5d,0,2,11,2.8d,12, facing));
        } else if (value == 4) {
            shapes.add(rotateBox(2.5d,0,2,11,2.8d,12, facing));
        }
        return shapes;
