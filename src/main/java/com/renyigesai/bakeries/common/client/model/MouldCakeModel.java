...
import com.renyigesai.bakeries.common.items.CustomCakeItem;
import com.renyigesai.bakeries.common.items.MouldCakeBlockItem;
...
public class MouldCakeModel implements BakedModel {
    ...
    private final ItemOverrides itemOverrides = new ItemOverrides() {
        ...
        public BakedModel resolve(BakedModel model, ItemStack stack, @Nullable ClientLevel level, @Nullable LivingEntity entity, int seed) {
            if (stack.getItem() instanceof MouldCakeBlockItem mcb) {
                Item item = mcb.mouldContent.get();
                if (item instanceof BlockItem blockItem){
                    return new CompositeCakeModel(originalModel,blockItem);
                }
    ...
    public MouldCakeModel(BakedModel originalModel) {
    ...
    public static class CompositeCakeModel implements BakedModel{
        ...
        public CompositeCakeModel(BakedModel model,BlockItem blockItem) {
            this.model = model;
            ...
            addModelQuads(model,pose,0f);
            BakedModel model1 = Minecraft.getInstance().getBlockRenderer().getBlockModel(blockItem.getBlock().defaultBlockState());
            addModelQuads(model1,pose,0.0625f);
        }

        private void addModelQuads(BakedModel model,PoseStack pose,float offsetY){
            List<BakedQuad> quads1 = new ArrayList<>();
            pose.translate(0,offsetY,0);
            quads1.addAll(model.getQuads(null, null, RandomSource.create(), ModelData.EMPTY, null));
            for (Direction dir : Direction.values())
                quads1.addAll(model.getQuads(null, dir, RandomSource.create(), ModelData.EMPTY, null));
    ...
    private void addModelQuads(BakedModel model) {
        List<BakedQuad> quads = new ArrayList<>();
        quads.addAll(model.getQuads(null, null, RandomSource.create(), ModelData.EMPTY, null));
        for (Direction dir : Direction.values())
            quads.addAll(model.getQuads(null, dir, RandomSource.create(), ModelData.EMPTY, null));

        for (BakedQuad quad : quads) {
            BakedQuad transformed = transformQuad(quad, new PoseStack(), true); // 恒等变换，保持原始
            if (quad.getDirection() == null)
                genQuads.add(transformed);
            else
                faceQuads.get(quad.getDirection()).add(transformed);
