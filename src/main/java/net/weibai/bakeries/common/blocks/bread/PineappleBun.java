...
public class PineappleBun extends BreadBlock {
    @Override
    public List<VoxelShape> createShapes(@NotNull BlockState state, @NotNull BlockGetter level, @NotNull BlockPos pos, @NotNull CollisionContext context) {
        ...
        if (value == 1) {
            shapes.add(rotateBox(6,0,5.5d,4,2.3d,4, facing));
        } else if (value == 2) {
            shapes.add(rotateBox(3,0,4.5d,10,2.3d,7, facing));
        } else if (value == 3) {
            shapes.add(rotateBox(3,0,2,10,2.3d,12, facing));
        } else if (value == 4) {
            shapes.add(rotateBox(3,0,3,10,2.3d,11, facing));
        }
        return shapes;
    }
}
